...
export const ResourceDashboard: React.FC<ResourceDashboardProps> = ({
  ...
  onUpdateProject, onShowToast, onUpdateUser, themeColor = 'teal'
}) => {
  ...
  const avgUtil = pmStats.length > 0
    ? Math.round(pmStats.reduce((s, p) => s + p.utilizationPct, 0) / pmStats.length) : 0;
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {pendingRequests.map(project => {
              ...
              return (
                <div key={project.id} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-bold text-slate-900 text-sm">{project.clientName}</p>
                      <p className="text-[10px] text-slate-400 uppercase tracking-widest mt-0.5">{project.packageName}</p>
                    </div>
                  ...
                  <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                    ...
                    <p className="text-[10px] text-slate-400 mt-1.5 uppercase tracking-wider">by {req.requestedBy}</p>
                  </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={s.label} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
            ...
            <p className={cn('text-2xl font-black mt-1', s.warn ? 'text-rose-600' : 'text-slate-900')}>{s.value}</p>
          </div>
        ))}
      </div>
