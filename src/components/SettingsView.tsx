...
const PackageServiceConfig = ({ config, setConfig, theme, showToast }: any) => {
  ...
  const handleSaveService = (id: string) => {
    ...
    const parsedForm = parseMilestoneStrings(editForm);
    const updated = config.serviceBaselines.map((s: any) => s.id === id ? parsedForm : s);
  ...
  const handleAddService = () => {
    ...
    const parsedForm = parseMilestoneStrings(newForm);
    const newService = { id: Math.random().toString(36).substr(2, 9), ...parsedForm, subServices: parsedForm.subServices || [] };
              ...
              <div key={service.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                ...
                <div className="flex items-center justify-between px-5 py-4">
                  <div className="flex items-center gap-3 flex-1 min-w-0 flex-wrap">
                    ...
                    <span className="px-2 py-0.5 bg-slate-100 text-slate-500 text-[10px] font-black rounded-md">{service.milestones?.length || 0} milestones</span>
                  </div>
                  {isEditing ? (
                    <div className="flex gap-2 flex-shrink-0">
                      <button onClick={() => handleSaveService(service.id)} className={cn("px-3 py-1.5 text-white text-xs font-bold rounded-lg", theme.bg)}>Save</button>
                ...
                {/* Default Milestones (shown in edit mode) */}
                {isEditing && (
                  <div className="px-5 pb-4 space-y-1.5 border-t border-slate-100 pt-3">
                    <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Default Milestones (comma-separated, used when no sub-service)</label>
                    <input className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-teal-500/20" placeholder="Setup, Test, Go-Live" value={editForm.milestones ? (Array.isArray(editForm.milestones) ? editForm.milestones.join(', ') : editForm.milestones) : ''} onChange={e => setEditForm({ ...editForm, milestones: e.target.value })} />
                  </div>
                )}
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="bg-slate-50/70">
                        ...
                        <th className="px-5 py-2.5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Milestones (comma-separated)</th>
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {subServices.map((ss: any, i: number) => (
                        <tr key={ss.id || i}>
                          ...
                          <td className="px-5 py-2.5">
                            {isEditing ? (
                              <input className="w-full px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs outline-none focus:ring-2 focus:ring-teal-500/20" placeholder="Step 1, Step 2..." value={ss.milestones ? (Array.isArray(ss.milestones) ? ss.milestones.join(', ') : ss.milestones) : ''} onChange={e => { const u = [...subServices]; u[i] = { ...ss, milestones: e.target.value }; setEditForm({ ...editForm, subServices: u }); }} />
                            ) : (
                              <span className="text-[10px] text-slate-400">{ss.milestones?.length ? `${ss.milestones.length} steps` : '—'}</span>
                            )}
                          </td>
                          {isEditing && (
                      ...
                      {isEditing && (
                        <tr>
                          <td colSpan={4} className="px-5 py-3">
                            <button onClick={() => { const newSS = { id: Math.random().toString(36).substr(2, 9), name: '', baselineDays: service.baselineDays || 1, milestones: [] }; setEditForm({ ...editForm, subServices: [...(editForm.subServices || []), newSS] }); }} className="text-[10px] font-black uppercase tracking-widest text-teal-600 hover:text-teal-700 flex items-center gap-1.5 transition-colors">
                      ...
                      {!isEditing && subServices.length === 0 && (
                        <tr>
                          <td colSpan={3} className="px-5 py-3 text-[10px] font-bold text-slate-400 italic">No sub-services configured — click the edit icon to add gateways or variants.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
