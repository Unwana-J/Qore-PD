...
const PackageServiceConfig = ({ config, setConfig, theme, showToast }: any) => {
  ...
  const [deliverablesModal, setDeliverablesModal] = useState<{ isOpen: boolean; serviceId: string | null; subServiceId?: string | null; serviceName: string; initialDeliverables: string[] }>({
    isOpen: false,
    serviceId: null,
    subServiceId: null,
    serviceName: '',
  ...
  const handleSaveDeliverables = async (deliverables: string[]) => {
    if (!deliverablesModal.serviceId) return;
    
    let updated;
    if (deliverablesModal.subServiceId) {
      updated = config.serviceBaselines.map((s: any) => {
        if (s.id === deliverablesModal.serviceId) {
          const updatedSubServices = (s.subServices || []).map((ss: any) => 
            ss.id === deliverablesModal.subServiceId ? { ...ss, deliverables } : ss
          );
          return { ...s, subServices: updatedSubServices };
        }
        return s;
      ...
      updated = config.serviceBaselines.map((s: any) => 
        s.id === deliverablesModal.serviceId ? { ...s, deliverables } : s
      );
    }

    setConfig({ ...config, serviceBaselines: updated });
    setDeliverablesModal({ ...deliverablesModal, isOpen: false, serviceId: null, subServiceId: null, serviceName: '', initialDeliverables: [] });
    showToast("Deliverables updated.");
  };
    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-300">
      ...
      {subTab === 'packages' ? (
        ...
        <div className="space-y-3">
              ...
              <div key={service.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                ...
                <div className="flex items-center justify-between px-5 py-4">
                  <div className="flex items-center gap-3 flex-1 min-w-0 flex-wrap">
                    ...
                    <button 
                      onClick={() => setDeliverablesModal({ isOpen: true, serviceId: service.id, serviceName: service.name, initialDeliverables: service.deliverables || [] })}
                      className="px-2 py-0.5 bg-slate-100 hover:bg-slate-200 text-slate-500 text-[10px] font-black rounded-md flex items-center gap-1 transition-colors"
                    >
                      <ListChecks className="w-3 h-3" />
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="bg-slate-50/70">
                        ...
                        <th className="px-5 py-2.5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Custom Deliverables</th>
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {subServices.map((ss: any, i: number) => (
                        <tr key={ss.id || i}>
                          ...
                          <td className="px-5 py-2.5 text-center">
                            <button 
                              onClick={() => setDeliverablesModal({ 
                                isOpen: true, 
                                serviceId: service.id, 
                                subServiceId: ss.id,
                                serviceName: `${service.name} (${ss.name})`, 
                                initialDeliverables: ss.deliverables || [] 
                              })}
                              className={cn(
                                "px-2 py-0.5 rounded text-[10px] font-black transition-colors flex items-center gap-1 mx-auto",
                                ss.deliverables?.length ? "bg-teal-50 text-teal-700 hover:bg-teal-100" : "bg-slate-50 text-slate-400 hover:bg-slate-100"
                              )}
                            >
                              <ListChecks className="w-3 h-3" />
                              {ss.deliverables?.length || 0} Set
                            </button>
                          </td>
                          {isEditing && (
                      ...
                      {isEditing && (
                        <tr>
                          <td colSpan={4} className="px-5 py-3">
                            <button onClick={() => { const newSS = { id: Math.random().toString(36).substr(2, 9), name: '', baselineDays: service.baselineDays || 1, milestones: [], deliverables: [] }; setEditForm({ ...editForm, subServices: [...(editForm.subServices || []), newSS] }); }} className="text-[10px] font-black uppercase tracking-widest text-teal-600 hover:text-teal-700 flex items-center gap-1.5 transition-colors">
                      ...
                      {!isEditing && subServices.length === 0 && (
                        <tr>
                          <td colSpan={5} className="px-5 py-3 text-[10px] font-bold text-slate-400 italic">No sub-services configured — click the edit icon to add gateways or variants.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
          ...
          <MilestoneEditorModal 
            isOpen={deliverablesModal.isOpen}
            onClose={() => setDeliverablesModal({ ...deliverablesModal, isOpen: false })}
            onSave={handleSaveDeliverables}
            initialMilestones={deliverablesModal.initialDeliverables}
            serviceName={deliverablesModal.serviceName}
            title="Configure Deliverables"
            label="Client-Facing Deliverable Pool"
