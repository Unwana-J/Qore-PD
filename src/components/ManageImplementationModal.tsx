...
export const ManageImplementationModal: React.FC<ManageImplementationModalProps> = ({
  ...
  const [suspensionDuration, setSuspensionDuration] = useState<number>(14);
  ...
  const handleApproveReactivation = async () => {
    ...
    try {
      await api.serviceExtensions.approveReactivation(extension.id, userName);
      onShowToast('Implementation reactivated.');
      onClose();
    } catch (err: any) {
      onShowToast(err.message, 'error');
    } finally {
  ...
  const handleRejectReactivation = async () => {
    ...
    setProcessingReactivation(true);
    try {
      await api.serviceExtensions.rejectReactivation(extension.id, rejectionReason, userName);
      onShowToast('Reactivation request rejected.');
      onClose();
    } catch (err: any) {
      onShowToast(err.message, 'error');
    } finally {
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
              ...
              {extension.status !== 'Completed' && (
                <div className="px-8 mb-6 space-y-3">
                  {/* Active Actions */}
                  {!isSuspended && !extension.extensionRequest && !extension.suspensionRequest && (
                    <>
                      ...
                      {(extension.mappingStatus === 'None' || extension.mappingStatus === 'Rejected' || extension.mappingStatus === 'Unmapped') && (
                        !showSuspensionModal ? (
                          <button
                            ...
                            disabled={!milestones[0]?.completed}
                            title={!milestones[0]?.completed ? 'You must complete the first milestone before requesting a suspension' : ''}
                            className={cn("w-full py-3 border rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2", 
                              !milestones[0]?.completed 
                                ? "bg-slate-50 border-slate-200 text-slate-300 cursor-not-allowed" 
                                : "bg-slate-50 border-slate-200 text-slate-400 hover:text-orange-600 hover:border-orange-200 hover:bg-orange-50/30"
                            )}
                          >
                          ...
                          <div className="p-4 bg-orange-50 border border-orange-200 rounded-2xl space-y-3 animate-in fade-in">
                            ...
                            <div className="space-y-2">
                              <select 
                                ...
                                className="w-full px-3 py-2 bg-white border border-orange-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-orange-500/20"
                              >
                                <option value={14}>Suspend for 14 Days</option>
                                <option value={30}>Suspend for 30 Days</option>
                                <option value={60}>Suspend for 60 Days</option>
                              </select>
                              <textarea
                            ...
                            <div className="flex gap-2">
                              <button onClick={() => { setShowSuspensionModal(false); setSuspensionReason(''); setSuspensionDuration(14); }} className="px-4 py-2 text-slate-500 text-xs font-bold rounded-lg hover:bg-slate-100 transition-colors">Cancel</button>
                              <button
                                onClick={async () => {
                                  if (!suspensionReason.trim()) return;
                                  setProcessingSuspension(true);
                                  try {
                                    await api.serviceExtensions.requestSuspension(extension.id, suspensionReason, suspensionDuration, userName);
                  ...
                  {/* Suspended Actions */}
                  {isSuspended && !extension.reactivationRequest && (
                    !showReactivationModal ? (
                      <button
                        onClick={() => setShowReactivationModal(true)}
                        className="w-full py-3 bg-emerald-50 border border-emerald-200 rounded-2xl text-[10px] font-black uppercase tracking-widest text-emerald-600 hover:bg-emerald-100 transition-all flex items-center justify-center gap-2"
                      >
                        <RefreshCw className="w-3.5 h-3.5" /> Request Reactivation
                      </button>
                    ) : (
                      <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl space-y-3 animate-in fade-in">
                        <p className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">Reactivation Request</p>
                        <textarea
                          className="w-full px-3 py-2.5 bg-white border border-emerald-200 rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400 transition-all resize-none"
                          placeholder="Provide a reason for reactivation (required)..."
                          rows={3}
                          value={reactivationReason}
                          onChange={e => setReactivationReason(e.target.value)}
                        />
                        <div className="flex gap-2">
                          <button onClick={() => { setShowReactivationModal(false); setReactivationReason(''); }} className="px-4 py-2 text-slate-500 text-xs font-bold rounded-lg hover:bg-slate-100 transition-colors">Cancel</button>
                          <button
                            onClick={async () => {
                              if (!reactivationReason.trim()) return;
                              setProcessingReactivation(true);
                              try {
                                await api.serviceExtensions.requestReactivation(extension.id, reactivationReason, userName);
                                onShowToast('Reactivation request submitted for approval.');
                            ...
                            disabled={processingReactivation || !reactivationReason.trim()}
